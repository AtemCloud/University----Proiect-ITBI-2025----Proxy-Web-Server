...
### Option B: Bind-mount a disk directory (persistent)
1. Create backing dir:
   ```bash
   sudo mkdir -p /var/cache/proxy
   sudo chown -R www-data:www-data /var/cache/proxy
   sudo chmod -R 750 /var/cache/proxy
   ```
2. Add to `/etc/fstab`:
   ```fstab
   /var/cache/proxy /mnt/proxy-cache none bind 0 0
   ```
3. Apply:
   ```bash
   sudo mount -a
   ls -ld /mnt/proxy-cache
   ```

### Option C: Dedicated partition (ext4)
1. Format (if needed):
   ```bash
...
2. Add to `/etc/fstab`:
   ```fstab
   /dev/sdb1 /mnt/proxy-cache ext4 defaults,noatime 0 2
   ```
...
Create `/etc/systemd/system/mnt-proxy-cache.mount`:
```ini
[Unit]
Description=Mount for proxy cache

[Mount]
What=tmpfs
Where=/mnt/proxy-cache
Type=tmpfs
Options=size=512M,mode=0750,uid=33,gid=33
...
sudo systemctl enable --now mnt-proxy-cache.mount
```
Note: `uid=33,gid=33` is `www-data`.
